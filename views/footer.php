<!-- footer.php -->
<footer class="footer-personalizado text-white mt-5 py-4" role="contentinfo">
  <div class="container">
    <div class="row text-center text-md-start">
      
      <!-- Columna 1: Derechos -->
      <div class="col-md-4 mb-3">
        <p>&copy; 2025 <strong>Vacunacion.info</strong> – Todos los derechos reservados.</p>
        <nav class="social-icons mt-2" aria-label="Redes sociales">
          <a href="#" target="_blank" rel="noopener" title="Visítanos en Facebook" aria-label="Facebook">
            <i class="bi bi-facebook text-white"></i>
          </a>
          <a href="#" target="_blank" rel="noopener" title="Síguenos en Twitter" aria-label="Twitter">
            <i class="bi bi-twitter text-white"></i>
          </a>
          <a href="#" target="_blank" rel="noopener" title="Síguenos en Instagram" aria-label="Instagram">
            <i class="bi bi-instagram text-white"></i>
          </a>
        </nav>
      </div>

      <!-- Columna 2: Mapa del sitio -->
      <div class="col-md-4 mb-3">
        <h5 id="mapaSitio">Mapa del sitio</h5>
        <ul class="list-unstyled" aria-labelledby="mapaSitio">
          <li><a href="/index.php" class="text-white text-decoration-none">Inicio</a></li>
          <li><a href="/views/calculadora.php" class="text-white text-decoration-none">Calculadora de vacunas</a></li>
          <li><a href="/views/calendariosVacunacion.php" class="text-white text-decoration-none">Calendario de vacunación</a></li>
          <li><a href="/views/preguntasFrecuentes.php" class="text-white text-decoration-none">Preguntas frecuentes</a></li>
          <li><a href="/views/enlaces.php" class="text-white text-decoration-none">Enlaces de interés</a></li>
          <li><a href="/views/about.php" class="text-white text-decoration-none">Sobre nosotros</a></li>
        </ul>
      </div>

      <!-- Columna 3: Enlaces útiles -->
      <div class="col-md-4 mb-3">
        <h5 id="enlacesUtiles">Enlaces útiles</h5>
        <ul class="list-unstyled" aria-labelledby="enlacesUtiles">
          <li><a href="/views/privacidad.php" class="text-white text-decoration-none">Política de privacidad</a></li>
          <li><a href="/views/cookies.php" class="text-white text-decoration-none">Política de cookies</a></li>
        </ul>
      </div>

    </div>
  </div>
</footer>

<script src="/views/javascript/register-sw.js"></script>
<script src="/views/javascript/install-pwa-button.js"></script>
<script src="/views/javascript/registro.js"></script>
<script src="/views/javascript/slider.js"></script>
<script src="/views/javascript/calculadora.js"></script>
<script src="/views/javascript/buscadorHeader.js"></script>
<script src="/views/javascript/errorLogin.js"></script>
<script src="/views/javascript/login.js"></script>

</body>
</html>
